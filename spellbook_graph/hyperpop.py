...
class HyperPopulation:
    ...
    def kick_decks(self, base: np.ndarray, P: int) -> np.ndarray:
        ...
        for p in range(P):
            X[self.rng.choice(inside, self.kick, replace=False), p] = 0
            X[self.rng.choice(outside, self.kick, replace=False), p] = 1
        return X
    ...
    def _bookkeep(self, best_now: np.ndarray, fetch_best_col) -> None:
        ...
        if stale.size:
            ...
            cols = np.concatenate([self.kick_decks(self.best_deck, half) if half else np.zeros((self.n, 0), np.float32),
                                   self.random_decks(stale.size - half)], axis=1)
            self._reseed(stale, cols)
